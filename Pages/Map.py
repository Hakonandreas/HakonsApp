...
def style_function(feature):
    if st.session_state.selected_area == feature["properties"]["ElSpot_omraade"]:
...
folium.GeoJson(
    ...
    tooltip=folium.GeoJsonTooltip(fields=["ElSpot_omraade"], aliases=["Price area:"])
).add_to(m)

# Function to find selected area from click
def handle_click(lat, lon):
    ...
    for feature in geojson_data["features"]:
        ...
        if polygon.contains(point):
            st.session_state.selected_area = feature["properties"]["ElSpot_omraade"]
            break
    else:
        st.session_state.selected_area = None
